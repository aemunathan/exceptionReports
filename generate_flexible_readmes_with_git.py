...
def get_git_metadata(path):
    ...
    try:
        ...
        latest_commit = subprocess.getoutput(f"git -C {path} log -1 --pretty=format:'%h - %s (%ci)'")
        branch = subprocess.getoutput(f"git -C {path} rev-parse --abbrev-ref HEAD")
        return {
            "remote_url": url.strip(),
            "last_commit": latest_commit.strip(),
            "branch": branch.strip()
        }
    except:
        return {}

def write_readme(path, name, proj_type, meta, structure, git_info):
    lines = [f"# {name}\n", f"## Project Type\n{proj_type}\n"]
...
def main():
    ...
    for entry in os.scandir(BASE_DIR):
        ...
        write_readme(project_path, name or entry.name, proj_type, meta, structure, git_info)
